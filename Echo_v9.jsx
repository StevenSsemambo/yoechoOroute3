...
// ── SUPABASE CONFIG ───────────────────────────────────────────────────────────
// ── PERSISTENT STORAGE (window.storage — Claude artifact built-in) ───────────
// window.storage is a sandboxed key-value store that persists across sessions.
// Keys are per-user by default (shared=false). Max 5MB per key.
const store = {
  async get(key) {
    try {
...
async function callClaude(messages, system){
  const res = await fetch("https://api.anthropic.com/v1/messages",{
    ...
    body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:8192, system, messages: buildMessageWindow(messages) })
  });
  if(!res.ok){ const e=await res.json().catch(()=>({})); const err=new Error(e?.error?.message||`API ${res.status}`); err.status=res.status; throw err; }
  const d=await res.json(); return d.content?.find(b=>b.type==="text")?.text||"I'm here.";
}
...
// ── MAIN APP ──────────────────────────────────────────────────────────────────
export default function YoEcho(){
