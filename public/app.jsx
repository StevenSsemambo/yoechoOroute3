...
const SUPABASE_URL  = 'https://xxxx.supabase.co';   // e.g. https://xxxx.supabase.co
...
// No Supabase keys in the browser. All cloud reads/writes go through the
// server-side Netlify function at /.netlify/functions/db which holds the keys
// securely in Netlify environment variables.
//
// Read flow:  localStorage first (instant/offline) → db function → cache locally
// Write flow: localStorage immediately (UI stays fast) → db function in background
const store = {
  async get(key) {
    // 1. Try localStorage first — instant and works offline
    try {
      const raw = localStorage.getItem(key);
      if (raw !== null) return JSON.parse(raw);
    } catch(e) { /* corrupt entry — fall through to server */ }

    // 2. Fall back to server-side Supabase via Netlify function
    try {
    ...
    } catch(e) { console.warn('[YoEcho] db.get failed:', e); }

    return null;
  },

  async set(key, value) {
    // 1. Write locally immediately — UI never waits on the network
    try {
      localStorage.setItem(key, JSON.stringify(value));
    } catch(e) { console.warn('[YoEcho] localStorage.set failed:', e); }

    // 2. Sync to Supabase via server function in background — fire and forget
    fetch('/.netlify/functions/db', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ action: 'set', key, value }),
    }).catch(e => console.warn('[YoEcho] db.set sync failed:', e));
  },
};
